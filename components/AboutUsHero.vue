<template lang="">
     <div class="max-w-full px-4 mx-auto">
        <div class="mx-auto text-center max-w-7xl">
            <div class="relative flex items-center justify-center">
                <h1 class="text-[2.5rem] font-bold text-[#3E2CA3]">ABOUT US</h1>
                <div class="absolute left-[70%] text-2xl rotate-12 top-[20%] h-[25px]">
                    💜
                </div>
            </div>
            <p class="mt-[8px] font-semibold text-[11px]">Lorem ipsum dolor sit amet consectetur</p>
        </div>

        <div class="relative isolate z-10 mt-[15px]">
            <div class="overflow-hidden">
                <div class="px-4 pt-3 pb-10 mx-auto max-w-7xl">
                    <div class="max-w-full mx-auto overflow-hidden gap-x-8 lg:mx-0 lg:items-center">
                        <div class="flex justify-center gap-3 mt-3 lg:mt-0">
                            <div class="flex-none w-44 sm:pt-5 lg:pt-3">
                                <div class="flex justify-center">
                                    <svg class="size-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
                                        <path fill="#96030d" d="M85.84 33.6s-9.44 3.93-9.18 5.37S75.61 61 75.61 61l1.44 3.41s.91 2.98 3.42 4c2.91 1.18 4.77-.61 4.77-.61l4.97-4.35l7.56.37l6.81-6.69s4.11.22 6.5-3.5c2.35-3.64 1.12-7.87 1.12-7.87S91.03 30.44 90.53 31.19s-4.69 2.41-4.69 2.41"/><path fill="#f7153a" d="M104.95 51.59c-.81.84-14.72 2.43-14.72 2.43l-.16 3.35s1.87 1.95 4.47 2.03c2.25.07 4.55-1.16 4.55-.88s-1.4 2.99-3.83 3.35s-5.15-.64-5.23.12s-.36 2.79-.36 2.79s3.26 1.75 7.7.52c4.19-1.16 6.22-3.87 7.06-6.38c1.38-4.13 1.04-7.87.52-7.33"/><path fill="#cc0b1e" d="M83.53 50.39c-.54 1.46-.2 3.15.96 4.19c1.14 1.02 2.79 1.68 2.79 2.03c0 .36-.61 3.36-1.12 5.5c-.76 3.23-2.75 4.63-2.39 5.86c.24.84 1.78 1.66 3.99.6c2.07-1 3.51-2.43 3.95-7.34c.22-2.43.28-4.39.28-4.39s3.15.16 7.06-.88s6.78-3.44 6.46-5.38c-.34-2.06-4.4-1.08-7.9-.16c-2.77.73-6.18.92-8.58.24s-4.66-2.54-5.5-.27"/><path fill="#cd0e1e" d="M67.27 26.93s.21 4 .91 8.05c.8 4.64 1.83 9.54 1.4 13.53c-.53 4.86-1.95 12.17 2.07 15.47c3.92 3.22 5.86 1.38 5.86 1.38s-.33-3.4.79-5.38s2.79-3.8 3.67-8.92c.95-5.51-.57-8.85-.14-9.39s1.27-.03 2.5.62c1.23.64 6.99 4.2 11.65 5.3c5.58 1.32 13.9.5 16.8-.79c3.27-1.45 3.96-4.73 2.33-7.5c-1.77-2.99-5.78-3.61-8.31-6c-1.77-1.67-7.52-6.75-10.95-10.88c-3.44-4.13-5.82-4.93-5.82-4.93s-14.65-3.97-16.16-1.13s-6.6 10.57-6.6 10.57"/><path fill="#738136" d="M69.8 13.98s.01 3.57-3.47 4.83c-2.45.88-5.1.27-5.51.61c-.84.7.07 2.93 2.38 3.54c1.6.42 3.74.14 3.74.14s-.95 2.11-2.24 2.93s-2.99 1.2-2.86 1.77c.57 2.42 5.02 2.05 7.55.92c5.65-2.52 6.31-6.03 6.31-6.03s.9.58 2.12 2.34c1.22 1.77 4.39 7.19 5.58 5.75c1.36-1.63 1.29-5.37.54-7.82c-.76-2.47-1.5-3.74-1.5-3.74s2.75 1.45 6.46.82c4.53-.77 7.07-4.56 5.44-7.35c-.61-1.04-2.04.95-5.92 1.43c-1.35.17-3.55-.75-3.55-.75s1.8-1.49 1.8-5.05c0-2.73-2.46-5.07-4.45-4.71c-.48.09.31 3.84-1.88 5.9c-1.16 1.09-3.16 1.47-6.02.66c-1.08-.31-4.95-1.43-4.95-1.43s.17 2.71.43 5.24"/><path fill="#cd0e1e" d="M103.36 69.73c.33.99 1.15 4.15.93 8.99c-.18 3.87-3.12 6.17-3.71 7.97c-.45 1.37 9.86 2.35 11.27-6.03c1.1-6.57-2.51-9.8-4.13-11.03c-2.32-1.76-4.7-.92-4.36.1M85.55 96.06c-1.95.13-2.04 4.54-1.11 7.51s4.54 10.2 11.4 11.78s9.09-.46 9.18-1.58s-3.43-1.76-4.45-4.36c-.61-1.54-1.3-4.27-3.71-6.68s-6.12-2.5-7.88-3.71c-1.76-1.2-2.03-3.05-3.43-2.96"/><path fill="#748138" d="M26.77 54.61s-9.69-1.25-13.54 8.25c-3.34 8.25-.74 14 .93 18.27c1.13 2.9 2.31 7.92 3.06 8.07c.93.19 6.12-5.38 10.2-10.2s7.14-11.13 3.71-18.45c-1.96-4.17-4.36-5.94-4.36-5.94m24.11 17.34s2.04-4.17 5.38-4.27c2.79-.08 4.36 2.04 5.1 3.71s.56 2.5 0 4.27s-1.68 4.8-.56 8.53c1.11 3.71 4.91 8.16 7.32 10.29s7.97 6.4 8.81 6.12c.83-.28 1.21-9.74.74-15.02c-.46-5.29-2.5-9.83-6.12-11.87s-4.08-1.58-5.01-2.69s-2.41-7.05-9.27-8.16c-4.56-.74-8.81 2.23-8.81 2.69c.01.47 2.42 6.4 2.42 6.4"/><path fill="#748138" d="M77.38 12.64c-.19-.28-7.51-10.38-20.95-8.9S38.9 17.65 38.53 27.66c-.29 7.8.09 16.23 1.67 23.64c1.58 7.42 3.99 17.62 5.84 31.62c1.39 10.49 1.5 19.52 1.11 23.64c-.56 5.93-2.32 15.48-2.32 15.48s1.11 2.41 4.91 2.32c2.42-.06 3.15-1.76 3.15-1.76s1.95-9.55 1.58-25.22s-4.54-34.86-6.49-42.65s-5.47-28.09-.37-37.83s16.23-7.05 19.47-5.01s5.1 4.27 5.1 4.27z"/><path fill="#748138" d="M23.17 56.88c-.09-.28 1.28-6.99 6.75-11.07s10.94-1.3 10.94-1.3l1.58 8.34s-3.25-5.38-8.25-2.6c-5.91 3.28-5.71 8.39-5.71 8.39z"/></svg>
                                </div>
                                <p class="text-[10px] font-medium tracking-wider text-center">Lorem ipsum dolor sit amet</p>
                                <div class="relative flex justify-center">
                                    <img src="https://th.bing.com/th/id/OIP.3d7qYFnikrcpJceaX68CAAHaEJ?w=626&h=351&rs=1&pid=ImgDetMain"
                                        alt=""
                                        class="aspect-[3/3] w-full bg-gray-900/5 object-cover rounded-2xl shadow-lg h-[120px]">
                                    <div
                                        class="absolute inset-0 pointer-events-none rounded-xl ring-1 ring-inset ring-gray-900/10">
                                    </div>
                                </div>
                                <div class="flex justify-center">
                                    <svg class="size-8" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 36 36"><path fill="#fff" d="M15.632 34.661c-.799-.597-1.498-1.484-2.035-2.592l-.228-.47l-.46.249c-.975.528-1.913.858-2.744.969l-.202-3.756l-3.636.968c-.157-.854-.125-1.887.096-3.022l.103-.525l-.532-.066c-1.242-.154-2.306-.525-3.104-1.08l1.521-2.917l-2.988-1.523c.319-.944.948-1.882 1.834-2.735l.377-.363l-.379-.36c-.803-.764-1.408-1.554-1.77-2.311l3.51-1.353l-2.045-3.159c.74-.402 1.693-.686 2.789-.832l.519-.068l-.091-.514c-.215-1.211-.172-2.338.124-3.288l3.308.523l.524-3.308c.988.013 2.08.326 3.164.907l.462.248l.226-.473c.479-1.003 1.044-1.824 1.653-2.404L18 4.326l2.372-2.92c.609.58 1.175 1.401 1.653 2.404l.226.473l.462-.247c1.085-.581 2.178-.894 3.164-.906l.523 3.308l3.31-.525c.296.951.34 2.078.124 3.288l-.092.515l.518.069c1.095.145 2.048.43 2.788.832l-2.046 3.156l3.511 1.355c-.361.757-.966 1.547-1.77 2.311l-.379.36l.377.363c.888.854 1.516 1.793 1.835 2.736l-2.984 1.52l1.521 2.984c-.812.574-1.871.964-3.094 1.134l-.518.072l.096.514c.201 1.089.226 2.083.073 2.909l-3.634-.97l-.204 3.757c-.83-.11-1.768-.44-2.742-.968l-.459-.249l-.228.47c-.539 1.107-1.237 1.994-2.036 2.591L18 32.293z"/><path fill="#ea596e" d="M7.092 10.678c-.53-1.489-.698-2.97-.432-4.2l2.368.375l.987.156l.157-.988l.375-2.368c1.261.127 2.613.743 3.862 1.706c.118-.337.244-.663.382-.967c-1.551-1.135-3.223-1.763-4.73-1.763q-.184 0-.366.013l-.511 3.223l-3.224-.511c-.6 1.487-.565 3.415.085 5.393q.503-.056 1.047-.069m14.501-5.319c1.248-.962 2.6-1.578 3.86-1.705l.376 2.368l.156.988l.987-.157l2.369-.376c.266 1.23.098 2.71-.432 4.2q.542.013 1.046.07c.651-1.978.685-3.906.085-5.394l-3.225.512l-.511-3.224a6 6 0 0 0-.365-.012c-1.507 0-3.179.628-4.73 1.762q.209.46.384.968M7.368 27h.035c.067 0 .157-.604.26-.947c-.098.004-.197.046-.294.046c-1.496 0-2.826-.303-3.83-.89l1.089-2.128l.454-.887l-.891-.452l-2.136-1.088c.508-1.151 1.515-2.25 2.818-3.143a12 12 0 0 1-.81-.669c-1.687 1.217-2.846 2.755-3.235 4.31l2.908 1.483l-1.482 2.843C3.475 26.501 5.303 27 7.368 27m27.806-5.846c-.39-1.555-1.548-3.093-3.234-4.311c-.25.228-.523.451-.81.669c1.304.893 2.31 1.992 2.817 3.145l-2.136 1.088l-.891.453l.454.892l1.089 2.137c-1.004.587-2.332.904-3.828.904c-.099 0-.199-.01-.299-.013q.156.518.26 1.011l.039.002c2.066 0 3.892-.563 5.112-1.587l-1.482-2.908zm-12.653 9.182c-.447 1.517-1.181 2.812-2.119 3.651l-1.695-1.694l-.707-.707l-.707.707l-1.695 1.694c-.938-.839-1.673-2.136-2.12-3.652c-.296.206-.593.397-.886.563c.636 1.98 1.741 3.559 3.1 4.409L18 33l2.308 2.308c1.358-.851 2.464-2.428 3.101-4.408a12 12 0 0 1-.888-.564"/><path fill="#ea596e" d="M20.118 5.683c.426 1.146.748 2.596.841 4.284l.2 3.683l3.564-.946c1.32-.351 2.655-.536 3.86-.536q.24 0 .474.01l-1.827 2.819l3.139 1.211c-.958.759-2.237 1.514-3.814 2.123l-3.441 1.328l2.001 3.099c.918 1.42 1.509 2.782 1.838 3.96l-3.244-.865l-.182 3.357c-1.019-.677-2.132-1.66-3.198-2.973L18 23.374l-2.328 2.862c-1.066 1.312-2.179 2.295-3.198 2.972l-.18-3.354l-3.248.864c.329-1.178.921-2.54 1.839-3.961l2.004-3.099l-3.442-1.328c-1.577-.609-2.856-1.363-3.814-2.122L8.768 15l-1.827-2.823q.233-.01.473-.01c1.206 0 2.541.185 3.861.536l3.564.947l.202-3.683c.092-1.688.415-3.138.84-4.284L18 8.292zm.19-4.991L18 3.533L15.692.692c-1.989 1.532-3.421 4.992-3.646 9.112c-1.617-.43-3.192-.637-4.632-.637c-2.11 0-3.929.445-5.161 1.289l1.989 3.073l-3.415 1.316c.842 2.366 3.69 4.797 7.54 6.283c-2.241 3.465-3.116 7.106-2.407 9.516l3.537-.941l.196 3.654c2.512-.07 5.703-2.027 8.307-5.228c2.603 3.201 5.796 5.158 8.306 5.228l.198-3.655l3.535.943c.71-2.411-.165-6.05-2.404-9.517c3.849-1.485 6.696-3.918 7.538-6.283l-3.415-1.318l1.99-3.07c-1.233-.844-3.053-1.29-5.164-1.29c-1.438 0-3.013.207-4.63.636c-.225-4.119-1.657-7.579-3.646-9.111"/></svg>
                                </div>
                            </div>
                            <div class="flex-none mr-auto space-y-4 w-36 sm:mr-0 sm:pt-10 lg:pt-8">
                                <div class="relative ">
                                    <img src="https://cdn3d.iconscout.com/3d/premium/thumb/hand-holding-shopping-bag-9838587-8017855.png"
                                        alt=""
                                        class="aspect-[1/1.3] w-full rounded-2xl bg-gray-900/5 object-cover shadow-lg h-[140px]">
                                    <div
                                        class="absolute inset-0 pointer-events-none rounded-xl ring-1 ring-inset ring-gray-900/10">
                                    </div>
                                </div>
                                <div class="relative ">
                                    <img src="https://img.freepik.com/premium-photo/young-cheerful-african-american-woman-with-smile-holding-phone-stands-studio_580432-1228.jpg"
                                        alt=""
                                        class="aspect-[3/2] w-full rounded-2xl bg-gray-900/5 object-cover shadow-lg h-[100px]">
                                    <div
                                        class="absolute inset-0 pointer-events-none rounded-xl ring-1 ring-inset ring-gray-900/10">
                                    </div>
                                </div>
                            </div>
                            <div class="flex-none pt-5 space-y-8 w-44 sm:pt-0">
                                <div class="relative l h-[350px] w-[220px]">
                                    <img src="https://images.unsplash.com/photo-1670272504528-790c24957dda?ixlib=rb-4.0.3&ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&crop=left&w=400&h=528&q=80"
                                        alt=""
                                        class="aspect-[2/3] w-full h-[100%] rounded-2xl bg-gray-900/5 object-cover shadow-lg">
                                    <div
                                        class="absolute inset-0 pointer-events-none rounded-xl ring-1 ring-inset ring-gray-900/10">
                                    </div>
                                </div>
                            </div>
                            <div class="ml-[60px] w-32 flex-none space-y-4 sm:mr-0 sm:pt-20 lg:pt-16">
                                <div class="relative ">
                                    <img src="https://images.hindustantimes.com/rf/image_size_960x540/HT/p2/2017/01/17/Pictures/_b2157956-dc94-11e6-84f6-f9b2ee092ea6.jpg"
                                        alt=""
                                        class="aspect-[3/2] w-full rounded-2xl bg-gray-900/5 object-cover shadow-lg h-[100px]">
                                    <div
                                        class="absolute inset-0 pointer-events-none rounded-xl ring-1 ring-inset ring-gray-900/10">
                                    </div>
                                </div>
                                <div class="relative ">
                                    <img src="https://th.bing.com/th/id/OIP.anGAWhFCu5YXZr0FjGCX5QHaFX?w=1600&h=1161&rs=1&pid=ImgDetMain"
                                        alt=""
                                        class="aspect-[1/1.3] w-full rounded-2xl bg-gray-900/5 object-cover shadow-lg h-[140px]">
                                    <div
                                        class="absolute inset-0 pointer-events-none rounded-xl ring-1 ring-inset ring-gray-900/10">
                                    </div>
                                </div>
                            </div>
                            <div
                                class="flex-none w-32 pt-5 ml-auto space-y-8 sm:ml-0 sm:pt-20 lg:order-last xl:order-none xl:pt-32">
                                <div class="relative ">
                                    <img src="https://img.freepik.com/premium-photo/bill-shopping-cart-floating-purple-background-with-smartphone-shop-store-online-concept-cartoon-minimal-discount-promotion-sale-banner-website-3d-render-illustration_598821-1257.jpg"
                                        alt=""
                                        class="aspect-[3/3] w-full rounded-2xl  bg-gray-900/5 object-cover shadow-lg">
                                    <div
                                        class="absolute inset-0 pointer-events-none rounded-xl ring-1 ring-inset ring-gray-900/10">
                                    </div>
                                </div>
                                <div>
                                    <h2 class="font-bold mb-[8px] font-sans text-[1.1rem] text-[#3E2CA3]">Kainchi</h2>
                                    <p class="text-[10px] font-medium tracking-wider">Lorem ipsum dolor sit amet</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="flex justify-between mt-3 md:w-[55%] ml-auto relative ">
                        <div class="flex items-center justify-between w-full gap-2">
                        <div
                            class="bg-[#3E2CA3] whitespace-nowrap  mt-20 md:mt-10 text-[white] font-semibold tracking-[0.07em] flex items-center gap-2 cursor-pointer rounded-full justify-center py-[2px] px-3 h-[35px]">
                            <p class="text-[11px]  ">SCROLL DOWN</p>
                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m7 10l5 5l5-5"/></svg>
                        </div>
                        <div>
                           <svg class="size-16 absolute lg:left-auto left-[0%] lg:top-auto -top-[10%] rotate-12 text-gray-400" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 32 32"><path fill="currentColor" d="M29.02 11.754L8.416 9.474L7.16 4.715a.76.76 0 0 0-.727-.558H3.34a1.21 1.21 0 0 0-.963-.49a1.24 1.24 0 1 0 0 2.483c.4 0 .738-.2.965-.492h2.512l5.23 19.8a3.28 3.28 0 0 0-.89 2.242a3.29 3.29 0 0 0 3.292 3.293a3.296 3.296 0 0 0 3.297-3.293a3.2 3.2 0 0 0-.093-.743h5.533a3.3 3.3 0 0 0-.092.743c0 1.82 1.476 3.293 3.296 3.293S28.72 29.52 28.72 27.7a3.296 3.296 0 0 0-3.294-3.297c-.95 0-1.8.41-2.402 1.053h-7.136a3.28 3.28 0 0 0-2.402-1.053c-.38 0-.738.078-1.077.196l-.182-.686H26.81a2.5 2.5 0 0 0 2.39-1.96l1.575-7.798a2 2 0 0 0 .04-.414a1.995 1.995 0 0 0-1.795-1.988zm-3.592 16.24a.3.3 0 0 1-.297-.295c.003-.166.135-.298.298-.298s.295.132.297.297a.3.3 0 0 1-.297.294zm1.78-7.495l.948-.95l-.318 1.58zm-14.453-9.037L13.79 12.5l-1.29 1.29l-1.293-1.29l1.087-1.088zm4.498.498l.538.54l-1.29 1.29l-1.293-1.29l.688-.69l1.358.15zM9.63 14.076l.87-.868l1.29 1.292l-1.29 1.29l-.565-.563l-.304-1.152zm-.295-1.12l-.328-1.24l.785.785zM21.79 16.5l-1.29 1.29l-1.293-1.29l1.292-1.293zm-.583-2l1.292-1.292l1.29 1.292l-1.29 1.292zM18.5 15.79l-1.293-1.29l1.292-1.293l1.29 1.292zm-.71.71l-1.29 1.29l-1.292-1.29l1.292-1.293zm-3.29-.71l-1.293-1.29l1.292-1.293l1.29 1.292zm-.71.71l-1.29 1.29l-1.293-1.29l1.292-1.293zm-3.29.707l1.29 1.292l-.784.783l-.54-2.044l.033-.033zm.802 3.197l1.197-1.197l1.29 1.292l-1.29 1.29l-1.13-1.13zm1.906-1.905l1.29-1.293l1.293 1.292l-1.29 1.29l-1.292-1.29zm3.292.707l1.292 1.292l-1.292 1.29l-1.292-1.29zm.708-.708l1.292-1.293l1.29 1.292l-1.29 1.29zm3.29.707l1.293 1.292l-1.29 1.29l-1.292-1.292zm.71-.708l1.29-1.293l1.293 1.292l-1.29 1.29zm2-2l1.29-1.293l1.293 1.292l-1.29 1.29zm2-2l1.29-1.293L27.79 14.5l-1.29 1.292l-1.293-1.293zm-.71-.708l-1.155-1.156l2.082.23zM21.792 12.5l-1.29 1.292l-1.293-1.292l.29-.29l2.253.25zm-7.29-.71l-.152-.15l.273.03l-.12.12zm-4 .002l-.65-.65l1.17.13zm4 9.415l1.205 1.205h-2.41zm4 0l1.205 1.206h-2.412zm4 0l1.207 1.207h-2.414zm.707-.708l1.292-1.293l1.29 1.292l-1.29 1.29zm2-2l1.292-1.292l1.29 1.29l-1.29 1.293l-1.293-1.29zm3.292-.71l-1.292-1.29l1.29-1.292l.445.444l-.43 2.124l-.014.015zm.5-4.5l-.5.5l-.66-.657l1.017.112c.054.008.1.026.144.044zM13.488 27.993a.297.297 0 0 1 0-.593a.296.296 0 0 1 0 .591zm13.323-5.58h-1.517l1.207-1.207l.93.93c-.187.17-.423.29-.62.277"/></svg>
                        </div>
                          <div class="flex gap-3 mt-20 md:mt-10 ">
                          <div class="size-[35px] rounded-full flex items-center  justify-center cursor-pointer text-[#535353] hover:bg-[#212121] hover:text-white border border-[#535353] ">
                            <svg class="size-6" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="M10.733 19.79a.75.75 0 0 0 1.034-1.086L5.516 12.75H20.25a.75.75 0 0 0 0-1.5H5.516l6.251-5.955a.75.75 0 0 0-1.034-1.086l-7.42 7.067a1 1 0 0 0-.3.58a.8.8 0 0 0 .001.289a1 1 0 0 0 .3.579z"/></svg>
                          </div>
                          <div class="size-[35px] rotate-180 rounded-full flex items-center justify-center cursor-pointer text-[#535353] hover:bg-[#212121] hover:text-white border border-[#535353] ">
                            <svg class="size-6" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24"><path fill="currentColor" d="M10.733 19.79a.75.75 0 0 0 1.034-1.086L5.516 12.75H20.25a.75.75 0 0 0 0-1.5H5.516l6.251-5.955a.75.75 0 0 0-1.034-1.086l-7.42 7.067a1 1 0 0 0-.3.58a.8.8 0 0 0 .001.289a1 1 0 0 0 .3.579z"/></svg>
                          </div>
                        </div>
                        </div>



                        
                    </div>
                </div>

            </div>
        </div>
    </div>
    
</template>
<script>
export default {
    
}
</script>
<style lang="">
    
</style>